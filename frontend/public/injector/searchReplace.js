...
(function () {
   ...
   */
  const SearchReplaceEngine = {
    /**
     * 原子化行替换逻辑 - Line-Number Based Patching Protocol 核心
     */
    applyLinePatch: function (originalContent, startLine, endLine, replaceBlock) {
      const origLines = originalContent.split('\n');
      const replaceLines = replaceBlock.split('\n');
      const newLines = origLines.slice(0, startLine - 1).concat(replaceLines, origLines.slice(endLine));
      return newLines.join('\n');
    },

    /**
     * 安全地应用多个补丁 - 防止行号漂移
     * 核心策略：按 start_line 从大到小排序（倒序）
     */
    applyPatchesSafely: function (originalContent, patches) {
      const lines = originalContent.split('\n');

      // 按照 start_line 从大到小排序 (关键！防止行号漂移)
      const sortedPatches = patches.slice().sort(function (a, b) {
        return b.start_line - a.start_line;
      });

      for (let i = 0; i < sortedPatches.length; i++) {
        const p = sortedPatches[i];
     ...
     */
    _getSimilarityRatio: function (a, b) {
      if (a === b) return 1.0;
      ...
      const costs = new Array(shorter.length + 1).fill(0);
      ...
      for (let i = 0; i <= longer.length; i++) {
        ...
        for (let j = 0; j <= shorter.length; j++) {
          if (i === 0 || j === 0) {
            ...
            const cj = longer[i - 1] === shorter[j - 1] ? nw + 1 : Math.max(costs[j], costs[j - 1]);
            nw = costs[j];
            costs[j] = cj;
          }
        }
     ...
     */
    getBestMatchHint: function (originalLines, searchBlock, startLine) {
      if (!searchBlock.trim()) {
        return '提示：expected_original 为空，无法验证';
      }

      const searchLines = searchBlock
        .trim()
        .split('\n')
      ...
      for (let i = searchStart; i <= searchEnd - searchLines.length; i++) {
        ...
        const ratio = this._getSimilarityRatio(windowText, searchText);
        if (ratio > bestRatio) {
          bestRatio = ratio;
          bestMatchStart = i;
        }
     ...
     */
    applySearchReplace: function (original, searchBlock, replaceBlock, fallbackStart, fallbackEnd) {
      if (!searchBlock) {
      ...
      console.info(
        '[SearchReplace] 开始搜索替换: 原文件 ' +
          originalLinesCount +
          ' 行, 搜索块 ' +
          searchBlockLinesCount +
          ' 行'
      );
      ...
      if (occurrences > 1) {
        console.error(
          '[SearchReplace] 唯一性校验失败: search_block 在文件中出现 ' +
            occurrences +
            ' 次，必须确保唯一'
        );
        return null;
      ...
      if (searchLen > 0 && origLinesClean.length >= searchLen) {
        console.debug(
          '[SearchReplace] 第3级匹配: 清洗后原文件 ' +
            origLinesClean.length +
            ' 行, 搜索块 ' +
            searchLen +
            ' 行'
        );
        ...
        for (let i = 0; i <= origLinesClean.length - searchLen; i++) {
          ...
          if (JSON.stringify(window) === JSON.stringify(searchLinesClean)) {
            ...
            const newLines = origLines
              .slice(0, matchStart)
              .concat(replLines, origLines.slice(matchEnd + 1));
      ...
      if (fallbackStart !== undefined && fallbackEnd !== undefined) {
        ...
        console.info(
          '[SearchReplace] 第4级匹配: 尝试 fallback 行号 ' +
            fallbackStart +
            '-' +
            fallbackEnd +
            ' (文件共 ' +
        ...
        if (1 <= fallbackStart && fallbackStart <= fallbackEnd && fallbackEnd <= totalLines) {
          const newLines = lines
            .slice(0, fallbackStart - 1)
            .concat(replNorm.split('\n'), lines.slice(fallbackEnd));
          console.info(
            '[SearchReplace] 第4级匹配成功: fallback 行号替换 ' + fallbackStart + '-' + fallbackEnd
          );
          return newLines.join('\n');
        } else {
          console.warn(
            '[SearchReplace] 第4级匹配失败: fallback 行号 ' +
              fallbackStart +
              '-' +
              fallbackEnd +
              ' 超出范围 (文件共 ' +
     ...
     */
    flexibleSearchReplace: function (originalText, searchBlock, replaceBlock) {
      return this.applySearchReplace(originalText, searchBlock, replaceBlock, undefined, undefined);
    },
  };

  // 导出到全局
  window.OmniFlowAISearchReplace = SearchReplaceEngine;
})();
