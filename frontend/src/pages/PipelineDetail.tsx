...
function StageMetrics({ pipeline }: StageMetricsProps) {
  ...
  const getStageStats = (stage: PipelineStage) => {
    // 使用 created_at 和 completed_at 计算真实耗时
    let durationText = '-';
    let durationSeconds = 0;

    if (stage.created_at && stage.completed_at) {
    ...
    // 从 output_data 中提取真实的 Token 用量（如果后端提供了）
    const outputData = stage.output_data as Record<string, any> | undefined;
    let tokens = 0;
    if (outputData?.input_tokens || outputData?.output_tokens) {
      tokens = (outputData.input_tokens || 0) + (outputData.output_tokens || 0);
    } else if (outputData?.metrics) {
      tokens = (outputData.metrics.input_tokens || 0) + (outputData.metrics.output_tokens || 0);
    }
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-slate-900">
            {(pipeline.stages?.reduce((acc, s) => {
              const outputData = s.output_data as Record<string, any> | undefined;
              let tokens = 0;
              if (outputData?.input_tokens || outputData?.output_tokens) {
                tokens = (outputData.input_tokens || 0) + (outputData.output_tokens || 0);
              } else if (outputData?.metrics) {
