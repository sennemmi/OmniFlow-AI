...
export function PipelineDetail() {
  ...
  const handleTerminate = useCallback(async () => {
    ...
    try {
      ...
      // 刷新状态
      queryClient.removeQueries({ queryKey: ['pipeline', pipelineId] });
      queryClient.refetchQueries({ queryKey: ['pipeline', pipelineId] });
    } catch (error) {
    ...
    <div className="h-full flex flex-col relative">
      ...
      {isFetching && pipeline && (
        <div className="absolute top-0 left-0 right-0 h-0.5 bg-blue-100 overflow-hidden z-50">
          <div className="h-full w-1/3 bg-blue-500 animate-refresh-bar" />
        </div>
      )}
      {/* 头部 - 优化样式 */}
