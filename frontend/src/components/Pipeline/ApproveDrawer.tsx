...
export function ApproveDrawer() {
  ...
  const handleRejectAll = async () => {
    ...
    try {
      await apiPost<any>(`/pipeline/${selectedPipeline.id}/approve-code-review`, {
        approve_coding: false,
        approve_testing: false,
        feedback: feedback
      });
      ...
      setCoderDecision('reject');
      setTesterDecision('reject');

      queryClient.invalidateQueries({ queryKey: ['pipeline', selectedPipeline.id] });
      queryClient.invalidateQueries({ queryKey: ['pipelines'] });

      addToast({ type: 'warning', message: '已全部拒绝，流程回退中' });
