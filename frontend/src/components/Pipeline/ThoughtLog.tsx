...
import { Terminal, Cpu, Search, FileCode, AlertTriangle, CheckCircle, Sparkles, Pause, Info } from 'lucide-react';
...
interface LogEntry {
  id: string;
  timestamp: Date;
  type: 'info' | 'thinking' | 'action' | 'warning' | 'success' | 'error' | 'paused';
  message: string;
  details?: string;
}
...
export function ThoughtLog({ pipelineId, stageId, status, isRunning: initialIsRunning = true }: ThoughtLogProps) {
  ...
  useEffect(() => {
    ...
    es.onmessage = (event) => {
      try {
        const data = JSON.parse(event.data) as { ts: string; level: string; msg: string; stage: string };
