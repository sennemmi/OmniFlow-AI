...
class ToolUsingAgent(LangGraphAgent[T]):
    ...
    async def _call_llm_with_tools(
        ...
        while tool_call_count < self.MAX_TOOL_CALLS:
            try:
                ...
                # 确保 model 有 openai/ 前缀，让 litellm 正确路由
    ...
    async def execute(
        ...
        try:
            # 获取 project_path
            from app.core.config import settings
            project_path = initial_state.get("project_path") or settings.TARGET_PROJECT_PATH
            # Windows 路径兼容处理
            from pathlib import Path
            if project_path and not Path(project_path).exists():
                logger.warning(f"[{self.agent_name}] project_path 不存在: {project_path}")
            logger.info(f"[{self.agent_name}] 使用项目路径: {project_path}")
            ...
            if result.get("error") == "Max tool calls reached":
                # 最后一次不带工具直接让 LLM 输出结论
                logger.warning(f"[{self.agent_name}] 工具次数耗尽，尝试直接输出结论")
                from app.agents.llm_providers import get_llm_provider
                provider = get_llm_provider()
                fallback = await provider.call(
                    system_prompt=self.system_prompt + "\n\n【注意】项目目录为空或无法访问，请基于需求直接输出 JSON 方案，不要调用工具。",
                    user_prompt=self.build_user_prompt(initial_state)
                )
                result = {
