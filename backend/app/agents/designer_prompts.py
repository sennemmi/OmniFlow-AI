...
Auto-generated from designer.system_prompt property
"""
...
【API 端点签名强制规范 - 绝对遵守】
对于所有 API 端点（位于 app/api/v1/ 或 app/api/ 目录下的路由函数），signature 必须包含 `request: Request` 参数：
- ✅ 正确: "async def get_timestamp(request: Request) -> dict"
- ❌ 错误: "async def get_timestamp() -> dict"

原因：
1. FastAPI 中间件需要从 request.state 获取 request_id 用于统一响应
2. CoderAgent 需要按照此签名实现，TesterAgent 需要按照此签名生成测试调用
3. 缺少 request 参数会导致契约与实际实现不一致，测试运行时参数缺失

【规则】
- 如果 module 路径包含 "api/"，则该函数是 API 端点
- API 端点的 signature 必须包含 `request: Request`，且声明返回类型为 `-> ResponseModel`（禁止 `-> dict`） 作为第一个参数（在 self 或 cls 之后）
- 非 API 端点（如 service 层函数）不需要 request 参数
- expected_behavior: 简短行为描述（如 "返回数据库连接状态"）
- is_async: 是否为异步函数（true/false）
- return_type: API端点必须为 "ResponseModel"，服务函数可为 "dict" / "class" 等
...
{{
  "interface_specs": [
    {{
      ...
      "signature": "async def check_database_status(request: Request) -> ResponseModel",
      "expected_behavior": "返回数据库连接状态，包含 status 和 response_time_ms 字段",
      "is_async": true,
      "return_type": "ResponseModel",
      "covers_criteria": [1, 2],
      "return_fields": [
...
{{
  "interface_specs": [
    {{
      ...
      "signature": "async def get_user_profile(user_id: int, request: Request) -> ResponseModel",
