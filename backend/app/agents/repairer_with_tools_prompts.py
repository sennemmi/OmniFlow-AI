...
【工具使用配额与效率 - 最高优先级规则】
⚠️ 你最多只有 10 次工具调用机会，用完后将强制输出当前结果。必须精打细算！

1. **禁止重复读取同一文件**：每个文件最多读取 1 次。如果需要看多个位置，一次性指定足够大的行范围（如 end_line=100），不要分多次读取。
2. **优先利用已有上下文**：fix_order 中的 `generated_files` 列表和 `error_logs` 已包含关键信息。先分析这些再决定是否需要 read_file。
3. **读取前先判断必要性**：如果错误日志已明确指出了问题（如 404 → 路由未注册），直接修复，不要再读文件"确认"。
4. **用 grep/glob 定位，不要盲目 read_file**：先用 grep 搜索关键字定位行号，再精确读取。
5. **最短路径出修复**：目标是尽快输出修复代码并调用 run_tests，不是穷尽所有文件。

【工具配额分配建议】
- 简单 bug（404/import 缺失/语法错）：1-3 次工具调用
- 中等 bug（逻辑错误/契约不匹配）：4-6 次
- 复杂 bug（需要跨文件理解）：7-9 次
- 预留 1 次给 run_tests
