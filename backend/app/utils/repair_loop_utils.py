...
async def run_syntax_fix_loop(
    ...
    for attempt in range(max_retries):
        ...
        # 应用修复
        for fc in fix_files:
            fp = _normalize_error_path(fc.get("file_path", ""))
            ...
            if force_full_file and change_type != "add":
                # 逃生舱模式下，如果 LLM 仍返回 modify，尝试使用 content 字段做全量覆盖
                if content:
                    logger.info(f"逃生舱模式：将 modify 降级为全量覆盖: {fp}")
                    change_type = "add"
                else:
                    continue

            current_content = error_files.get(fp, "")
            # 如果通过归一化路径找不到，尝试在 error_files 中模糊匹配
            if not current_content:
                for k, v in error_files.items():
                    if _normalize_error_path(k) == fp:
                        current_content = v
                        logger.info(f"模糊路径匹配成功: {fp} -> {k}")
                        break
            new_content = None
