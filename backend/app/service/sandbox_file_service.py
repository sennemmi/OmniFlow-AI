...
class SandboxFileService:
    ...
    async def _write_file_safe(self, full_path: str, content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 将文件路径作为命令行参数传递 (sys.argv[2])，而非嵌入 Python 代码字符串
            # 避免 repr() 单引号在 sh -c 多层级引号中被错误处理
            cmd = (
                f"python3 -c \""
                f"import base64, sys; "
                f"data = base64.b64decode(sys.argv[1]); "
                f"open(sys.argv[2], 'wb').write(data)"
                f"\" {shlex.quote(encoded)} {shlex.quote(full_path)}"
            )
    ...
    async def _verify_python_syntax(self, full_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if check_result.exit_code != 0:
                # 2>&1 将 stderr 合并到 stdout，优先检查 stdout
                error_msg = check_result.stdout.strip() or check_result.stderr.strip()
                if not error_msg:
                    error_msg = f"语法检查失败 (exit_code={check_result.exit_code})"
