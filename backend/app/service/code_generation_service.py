    ...
    async def generate_and_fix(
        ...
        while attempt <= self.MAX_FIX_RETRIES:
            if attempt > 0:
                ...
                # 【修复】每次重试前，从沙箱重新读取 injected_files 的最新内容
                # 这样 CoderAgent 能看到上一次修复的结果，而不是原始版本
                if injected_files and file_service:
                    updated_injected_files = {}
                    for file_path, original_content in injected_files.items():
                        read_res = await file_service.read_file(file_path)
                        if read_res.exists:
                            updated_injected_files[file_path] = read_res.content
                        else:
                            updated_injected_files[file_path] = original_content
                    injected_files = updated_injected_files
                    await log("info", f"📂 已从沙箱重新加载 {len(injected_files)} 个注入文件")
