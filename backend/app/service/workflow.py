...
class WorkflowService:
    ...
    async def create_stage(
        ...
        如果该 pipeline 的同名阶段已存在（如拒绝后重试），则重用旧记录，
        重置状态为 RUNNING、更新输入数据，避免违反 UNIQUE 约束。
        """
        ...
        if existing:
            # 删除旧记录再创建新的，避免 SQLAlchemy session 状态导致的 INSERT 冲突
            ...
            stage.retry_count = (existing.retry_count or 0) + 1
            session.add(stage)
            await session.flush()
